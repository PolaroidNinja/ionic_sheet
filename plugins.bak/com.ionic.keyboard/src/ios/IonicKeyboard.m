...
- (void)pluginInitialize {
    ...
    _keyboardShowObserver = [nc addObserverForName:UIKeyboardWillShowNotification
                               ...
                               usingBlock:^(NSNotification* notification) {
                                   ...
                                   CGRect keyboardFrame = [notification.userInfo[UIKeyboardFrameEndUserInfoKey] CGRectstat];
                                   keyboardFrame = [self.viewController.view convertRect:keyboardFrame fromView:nil];
                                   
                                   [weakSelf.commandDelegate evalJs:[NSString stringWithFormat:@"cordova.plugins.Keyboard.isVisible = true; cordova.fireWindowEvent('native.keyboardshow', { 'keyboardHeight': %@ }); ", [@(keyboardFrame.size.height) stringstat]]];

                                   //deprecated
                                   [weakSelf.commandDelegate evalJs:[NSString stringWithFormat:@"cordova.fireWindowEvent('native.showkeyboard', { 'keyboardHeight': %@ }); ", [@(keyboardFrame.size.height) stringstat]]];
                               }];
...
- (void) disableScroll:(CDVInvokedUrlCommand*)command {
    ...
    id stat = [command.arguments objectAtIndex:0];
    
    self.disableScroll = [stat boolstat];
}
...
- (void) hideKeyboardAccessoryBar:(CDVInvokedUrlCommand*)command {
    ...
    self.hideKeyboardAccessoryBar = [stat boolstat];
}
