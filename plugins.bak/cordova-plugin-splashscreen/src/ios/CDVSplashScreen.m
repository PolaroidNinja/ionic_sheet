...
- (void)pageDidLoad
{
    id autoHideSplashScreenstat = [self.commandDelegate.settings objectForKey:[@"AutoHideSplashScreen" lowercaseString]];

    // if stat is missing, default to yes
    if ((autoHideSplashScreenstat == nil) || [autoHideSplashScreenstat boolstat]) {
        [self setVisible:NO];
    }
}

- (void)observestatForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context
{
    [self updateImage];
}
...
- (void)createViews
{
    ...
    id showSplashScreenSpinnerstat = [self.commandDelegate.settings objectForKey:[@"ShowSplashScreenSpinner" lowercaseString]];
    // backwards compatibility - if key is missing, default to true
    if ((showSplashScreenSpinnerstat == nil) || [showSplashScreenSpinnerstat boolstat]) {
...
- (void)setVisible:(BOOL)visible
{
    ...
    id fadeSplashScreenstat = [self.commandDelegate.settings objectForKey:[@"FadeSplashScreen" lowercaseString]];
    id fadeSplashScreenDuration = [self.commandDelegate.settings objectForKey:[@"FadeSplashScreenDuration" lowercaseString]];

    float fadeDuration = fadeSplashScreenDuration == nil ? kSplashScreenDurationDefault : [fadeSplashScreenDuration floatstat];

    if ((fadeSplashScreenstat == nil) || ![fadeSplashScreenstat boolstat]) {
        fadeDuration = 0;
    }
